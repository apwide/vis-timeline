 ...
 */
  ...
  setOptions(options) {
    if (options) {
      ...
      const fields = [
        ...
        'hide', 'snap', 'groupOrderSwap', 'groupOrderSwapLocalOnly', 'showTooltips', 'tooltip', 'tooltipOnItemUpdateTime', 'groupHeightMode', 'onTimeout'
      ];
  ...
  _onDragStart(event) {
    ...
    if (item && (item.selected || this.options.itemsAlwaysDraggable.item)) {
      ...
      const updateStartTime = item.editable == null || typeof item.editable.updateStartTime === 'undefined' || item.editable.updateStartTime
      const updateEndTime = item.editable == null || typeof item.editable.updateEndTime === 'undefined' || item.editable.updateEndTime
      if (dragLeftItem && !updateStartTime && !item.editable.updateGroup) {
        return;
      }
      if (dragRightItem && !updateEndTime && !item.editable.updateGroup) {
        return;
      }
      if (!dragLeftItem && !dragRightItem && (!updateStartTime || !updateEndTime) && !item.editable.updateGroup) {
        return;
      }
  ...
  _moveToGroup(item, groupId) {
    ...
    if (group && group.groupId != item.data.group) {
      const itemData = this._cloneItemData(item.data);
  ...
  _onGroupClick(event) {
    const group = this.groupFromTarget(event);
    this.body.emitter.emit('groupClicked', { group });
    setTimeout(() => {
  ...
  toggleGroupShowNested(group, force = undefined) {
    ...
    if (nestingGroup.showNested) {
      util.addClassName(group.dom.label, 'toggling')
    }
  ...
  _onGroupDragStart(event) {
    ...
    if (this.options.groupEditable.order) {
      ...
      let group = this.groupFromTarget(event);

      if (!group) {
        return;
      }
      if (group.editable && !group.editable.order) {
        return;
      }

      this.groupTouchParams.group = group;
      ...
      const groupsData = this.groupsData.getDataSet()
          ...
          if (group && group != this.groupTouchParams.group) {
              ...
              if (draggedGroup && targetGroup) {
                if (targetGroup.editable === false || (targetGroup.editable && !targetGroup.editable.order)) {
                  return;
                }
              ...
              if (!this.options.groupOrderSwapLocalOnly && !util.equalArray(newOrder, this.groupTouchParams.originalOrder)) {

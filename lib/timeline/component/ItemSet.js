  ...
  _onDragStart(event) {
    ...
    if (item && (item.selected || this.options.itemsAlwaysDraggable.item)) {
      ...
      const updateStartTime =
        item.editable == null ||
        typeof item.editable.updateStartTime === "undefined" ||
        item.editable.updateStartTime;
      const updateEndTime =
        item.editable == null ||
        typeof item.editable.updateEndTime === "undefined" ||
        item.editable.updateEndTime;
    ...
    if (this.options.groupEditable.order && this.groupTouchParams.group) {
      ...
      if (group && group != this.groupTouchParams.group) {
        ...
        if (draggedGroup && targetGroup) {
          if (
            targetGroup.editable === false ||
            (targetGroup.editable && !targetGroup.editable.order)
          ) {
            return;
          }
        ...
        if (
          !this.options.groupOrderSwapLocalOnly &&
          !util.equalArray(newOrder, this.groupTouchParams.originalOrder)
        ) {

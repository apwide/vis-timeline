 ...
 */
  ...
  /**
   * Set group editable properties if any
   * @param data
   */
  setEditable(data){
    if (data) {
      if (typeof data.editable === 'boolean') {
        this.editable = {
          add: data.editable,
          remove: data.editable,
          order: data.editable
        };
      } else if (typeof data.editable === 'object') {
        this.editable = {};
        util.selectiveExtend(['add', 'remove', 'order'], this.editable, data.editable);
      }
    }
  }
  ...
  _calculateHeight(margin) {
    ...
    if (items.length > 0) {
      ...
      height = 0 || this.props.label.height || this.dom.inner.clientHeight;
    }
    return height;
  }

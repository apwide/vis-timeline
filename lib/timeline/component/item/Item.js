...
class Item {
  ...
  _repaintDragCenter() {
    if (
      ...
      this.editable.updateStartTime &&
      this.editable.updateEndTime &&
      !this.dom.dragCenter
    ) {
      const me = this;
  ...
  _repaintOnItemUpdateTimeTooltip(anchor) {
    ...
    const isDraggingCurrentItem =
      this.parent.itemSet.touchParams.itemIsDragging &&
      (!this.parent.itemSet.touchParams.item ||
        this.id === this.parent.itemSet.touchParams.item.id);

    if (
      (this.selected || isDraggingCurrentItem) &&
      editable &&

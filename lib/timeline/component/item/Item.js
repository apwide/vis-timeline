 ...
 */
  ...
  setData(data) {
    ...
    if (groupChanged && this.parent != null) {
      data = this.parent.itemSet._moveToGroup(this, data.group);
    }
   ...
   */
  _repaintDragCenter() {
    if (this.selected && this.editable.updateTime && this.editable.updateStartTime && this.editable.updateEndTime && !this.dom.dragCenter) {
      const me = this;
  ...
  _repaintOnItemUpdateTimeTooltip(anchor) {
    ...
    const isDraggingCurrentItem = this.parent.itemSet.touchParams.itemIsDragging &&
      (!this.parent.itemSet.touchParams.item || this.id === this.parent.itemSet.touchParams.item.id)

    if ((this.selected || isDraggingCurrentItem) && editable && !this.dom.onItemUpdateTimeTooltip) {
    ...
    } else if (!this.selected && !isDraggingCurrentItem && this.dom.onItemUpdateTimeTooltip) {
      // remove button
    ...
    if (this.dom.onItemUpdateTimeTooltip) {
      ...
      if (content instanceof Element) {
        this.dom.onItemUpdateTimeTooltip.innerHTML = '';
        this.dom.onItemUpdateTimeTooltip.appendChild(content)
      } else {
    ...
    if (this.options) {
      if(typeof this.options.editable === 'boolean') {
        this.editable = {
          ...
          updateStartTime: this.options.editable,
          updateEndTime: this.options.editable,
      ...
      if (this.data) {
        if (typeof this.data.editable === 'boolean') {
          this.editable = {
            ...
            updateStartTime: this.data.editable,
            updateEndTime: this.data.editable,
          ...
          this.editable = {};
          util.selectiveExtend(['updateTime', 'updateStartTime', 'updateEndTime', 'updateGroup', 'remove'], this.editable, this.data.editable);
        }
      }
    }
  }
